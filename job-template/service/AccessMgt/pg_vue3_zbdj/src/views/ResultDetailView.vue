...
const mergedResultData = computed(() => {
  return resultDataStore.resultData
})
...
// 获取数据的方法
const fetchData = () => {
    reportTitle.value = mergedResultData.value.system_name
  // 数据现在通过props或路由参数传递，不再需要模拟数据
  if (mergedResultData.value && mergedResultData.value.tree_data) {
    const {tree_data, results, node_details} = mergedResultData.value
...
const initCharts = () => {
  ...
  if (barChart.value) {
    ...
    barInstance.value.setOption({
      ...
      series: [{ data: [], type: chartType.value, itemStyle: { color: '#409EFF' } }]
    })
  }
  ...
  if (lineChart.value) {
    ...
    lineInstance.value.setOption({
      ...
      xAxis: { type: 'category', data: [] },
      yAxis: { type: 'value', max: 100 },
      series: [{ data: [], type: 'line', symbol: 'circle', itemStyle: { color: '#409EFF' } }]
    })
  }
...
onMounted(() => {
  ...
  console.log('mergedResultData', mergedResultData.value)
})
